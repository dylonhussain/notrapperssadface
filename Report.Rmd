...
###Algorithm Selection 
Four popular machine learning algorithms were trained with the "train" partition and with different summary functions.The algorithms that were tested were Random Forest, KNN, Naive Bayes, and Negative Binomial Generalized Linear Mode labeled Rborist, knn, nb, and glm respectively. To assess the these models bootstrap partitions were created from the "train" partition and the algorithms were applied to these bootstrap partitions and the relevant average metrics were recorded (see table below). Note that the models labeled "no metric" used the default metric, accuracy.Sensitivity was the metric that was used to choose the algorithm because stars that are identified as pulsars would be further investigated so it is more important for the algorithm to identify potential pulsars than to reject stars that are not pulsars. 
...
## Conclusion
Multiple machine learning algorithms were tested for suitability to identify pulsars. Redundant predictors were identified and excluded to optimize computational efficiency. The optimal algorithm was found to be the Random Forest trained with a Beta=.8; this algorithm provides exceptional sensitivity and specificity and uses only four of the original eight predictors. This is a scalable algorithm that is suitable to identify pulsars. Future work on this problem may include the addition of additional predictors such as periodic variation and the addition of more observations would likely improve the algorithms' ability to identify pulsars. 